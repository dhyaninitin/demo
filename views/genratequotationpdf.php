...
include 'layouts/connection.php';
 require 'pdf/fpdf.php';
     ...
     $invoice_id=$_SESSION['newquotation'];
...
$res=mysqli_query($con,"SELECT * FROM newquotation WHERE id='$id'");
...
$y=$y+8;
	...
	$pdf->SetFillColor(51,153,255);
	$pdf->SetFontSize(11);
	$pdf->Cell(35,6,"Total Amount ",0,1,'C',1);
	...
	$pdf->SetFillColor(51,153,255);
	$pdf->SetFontSize(11);
	$pdf->Cell(50,6,"INR".'    '.  $row1['totalpayment'],0,1,'C',1);
...
if(!empty($points)){
  $totalpay =  $result . "Rupees  " . $points . " Paise";
   ...
   $totalpay = $result . "Rupees" ;
    ...
    $dir='newquotation-pdf/';
    $full_path = $dir . '/' . $filename;
    $pdf ->Output($full_path,'F');

unset($_SESSION["newquotation"]);
    header('location:newquotation-pdf/'.$filename);
?>